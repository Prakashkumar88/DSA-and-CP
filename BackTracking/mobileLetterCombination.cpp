...
void helper(int idx, string &digits, map<char, string> &mp, string &temp, vstr &ans)
{
    if (idx == len(digits))
    {
        ans.pb(temp);
    ...
    {
        temp += c;
        helper(idx + 1, digits, mp, temp, ans);
        temp.pop_back(); // backtrack to add more
    }
}
void solve()
{
    ...
    string temp;
    helper(0, digits, mp, temp, ans);

    trav(e, ans)
    {
        cout << e << sp;                     //Input: digits = "23"
