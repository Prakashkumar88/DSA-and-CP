...
int maxProfit(const vi &cost, const vi &w, int n, int m, vvi &dp) {
    if (n == 0 || m == 0) return 0;

    if (dp[n][m] != -1) return dp[n][m]; 

    if (w[n - 1] > m) {
        return dp[n][m] = maxProfit(cost, w, n - 1, m, dp);
    } else {
        return dp[n][m] = max(
            maxProfit(cost, w, n - 1, m, dp), 
            cost[n - 1] + maxProfit(cost, w, n - 1, m - w[n - 1], dp) 
        );
    }
}
...
void solve() {
    ...
    cout << maxProfit(cost, w, n, m, dp) << nl;
}
