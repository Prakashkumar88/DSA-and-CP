...
void solve()
{
    ...
    vll inputArray = {0};  // 1-based indexing: Add a dummy element at the start
    inputArray.insert(inputArray.end(), v.begin(), v.end()); // Fill the rest of the array
    SegmentTree segTree(inputArray);

    cout << "Maximum of range [1, 3]: " << segTree.query(1, 3) << nl;

    segTree.update(2, 10); 
    
    cout << "After update, maximum of range [1, 2]: " << segTree.query(1, 2) << nl;
}
...
int main()
{
    ...
    cin.tie(0);
                                                                                        //merge function require changes
                                                                    //query return according to merge function if min than retrun INT_MAX
    int t;
    cin >> t;
    while (t--)
        solve();
