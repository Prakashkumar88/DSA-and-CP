...
*/
vvi dp;
int lcsubstring(string &s1, string &s2, int n, int m, int &maxLen) {
    if (n == 0 || m == 0) return 0;

    if (dp[n][m] != -1) return dp[n][m];

    if (s1[n - 1] == s2[m - 1]) {
        dp[n][m] = 1 + lcsubstring(s1, s2, n - 1, m - 1, maxLen);
    ...
    lcsubstring(s1, s2, n - 1, m, maxLen); 
    lcsubstring(s1, s2, n, m - 1, maxLen);
