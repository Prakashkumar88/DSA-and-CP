...
vvi dp;
int isSubsequence(string s1, string s2, int n, int m)
{
    if (n == 0 || m == 0)
        return 0;
    ...
    if (s1[n - 1] == s2[m - 1])
    {
        return dp[n][m] = 1 + isSubsequence(s1, s2, n - 1, m - 1);
    }
    else
    {
        return dp[n][m] = max(isSubsequence(s1, s2, n, m - 1), isSubsequence(s1, s2, n - 1, m));
    }
...
void solve()
{
    ...
    // cout << isSubsequence(s1, s2, n, m) << nl;
    if(n == isSubsequence(s1, s2, n, m)) cout << "True\n";
    else cout << "False\n";
}
